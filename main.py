	...
	def __init__(self):
		...
		for shapes_of_the_cards in ['♥','♦','♣','♠']:
			for values_of_the_cards in range(2,11):
				self.pack.append(card(shapes_of_the_cards,values_of_the_cards))
...
#The show_pack method, traces each element (card) of the "pack" arrangement.
	def show_pack(self):
		for accountant in self.pack:
			print(str(accountant.values_of_the_cards) + accountant.shapes_of_the_cards)
...
#Instance "Chip" belonging to the "Money" class, which handles the amount of money wagered.
Chip = Money()
